...
module.exports = async ctx => {
  ...
  const path = await exists(configPath)
    ? configPath
    : await findConfigPath(configPath);
  if (!path) return;
  configPath = path;
  ...
  const yarnRootWorkspace = findYarnRootWorkspace(baseDir);
  const themeDirFromYarnNodeModules
    = yarnRootWorkspace !== null
    && join(yarnRootWorkspace, 'hexo-theme-' + theme);
  ...
  } else if (await exists(themeDirFromYarnNodeModules)) {
